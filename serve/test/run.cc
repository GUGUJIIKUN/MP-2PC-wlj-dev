...
static void execute_smallbank_split_2pc(
    ...
    txn_entry->distributed_branch_states.resize(actions.size());
    for (size_t i = 0; i < actions.size(); ++i) {
        txn_entry->distributed_branch_states[i].branch_id = static_cast<int>(i);
        txn_entry->distributed_branch_states[i].node_id = participants[i];
        txn_entry->distributed_branch_states[i].branch_txn_str = actions[i].sub_gid;
        txn_entry->distributed_branch_states[i].status = DistributedTxnStatus::INIT;
    }

    bool can_commit = true;
    std::vector<std::vector<page_id_t>> action_pages(actions.size());
    std::vector<std::unique_ptr<pqxx::connection>> single_prepared_conns(actions.size());
    std::vector<size_t> prepare_order(actions.size());
    for (size_t i = 0; i < prepare_order.size(); ++i) {
        prepare_order[i] = i;
    }
    std::sort(prepare_order.begin(), prepare_order.end(), [&](size_t lhs, size_t rhs) {
        const auto& left = actions[lhs];
        const auto& right = actions[rhs];
        return std::tie(participants[lhs], left.table_id, left.key, lhs) <
               std::tie(participants[rhs], right.table_id, right.key, rhs);
    });

    for (size_t order_idx = 0; order_idx < prepare_order.size(); ++order_idx) {
        const size_t i = prepare_order[order_idx];
        node_id_t participant_node_id = participants[i];
        const auto& action = actions[i];
        ...
        try {
            ...
            clock_gettime(CLOCK_MONOTONIC, &sub_end_time);
            prepare_ok = true;
            txn_entry->distributed_branch_states[i].status = DistributedTxnStatus::PREPARED;
            logger_->info("Txn id: " + std::to_string(tx_id) + " Sub-txn " + std::to_string(i) +
