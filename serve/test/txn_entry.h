...
enum class DistributedTxnStatus {
    INIT,
    // PREPARING,
    PREPARED,
    // COMMITTING,
...
struct DistributedTxnGlobalState {
    std::string txn_str;
    int participant_nodes = 0;
    DistributedTxnStatus status = DistributedTxnStatus::INIT;
    std::string error_message;
};

struct DistributedTxnBranchState {
    std::string branch_txn_str;  
    int branch_id = -1;
    int node_id = -1;
    DistributedTxnStatus status = DistributedTxnStatus::INIT;
};
...
struct TxnQueueEntry {
    ...
    DistributedTxnGlobalState distributed_global_state;
    std::vector<DistributedTxnBranchState> distributed_branch_states; 
};