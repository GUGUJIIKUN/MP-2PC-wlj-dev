...
public:
    explicit SmartRouter(const Config &cfg, TxnPool* txn_pool, std::vector<TxnQueue*> txn_queue, PendingTxnSet* pending_txn_queue_, int worker_threads,
            BtreeIndexService *btree_service, NewMetis* metis = nullptr, Logger* logger_ptr = nullptr, SynchronizedLogger* synchronized_logger = nullptr,
            SmallBank* smallbank = nullptr, YCSB* ycsb = nullptr, TPCC* tpcc = nullptr)
        : cfg_(cfg),
          txn_pool_(txn_pool),
          ...
          logger(logger_ptr),
          prepare_result_logger(synchronized_logger),
          ...
          tpcc_(tpcc)
    {
        ...
        if(SYSTEM_MODE <= 8 || SYSTEM_MODE == 13 || SYSTEM_MODE == 101 || (SYSTEM_MODE >= 23 && SYSTEM_MODE <= 25)) {
            router_worker_threads_ = std::min(worker_threads_, 32); // SYSTEM_MODE 0-8, 13, 101, 23-25 启动与工作线程数量相同的路由线程，但上限为32
    ...
    // SYSTEM_MODE 101 拆分子事务用：根据 (table_id, key) 当前记录的 ctid（即 hot_key_map 里
    // 该 key 所在的 page），在 ownership_table_ 里找到该 page 当前的 owner 节点。
    // 返回值：>=0 表示成功路由到该节点；-1 表示位置未知，调用方应自行兜底。
    // 该方法**不**修改 txn->accessed_page_ids，避免污染 update_key_page 的对齐断言。
    inline node_id_t route_subtxn_by_tuple_location(table_id_t table_id, itemkey_t key) {
        page_id_t page = -1;
        {
        ...
        if (page < 0 || page == kInvalidPageId) {
            return -1; // 没有记录该 key 的 ctid
        }
        // 边界保护，避免越界
        if (table_id < 0 || table_id >= MAX_DB_TABLE_NUM ||
            page >= MAX_DB_PAGE_NUM) {
            return -1;
        }
        auto owner_stats = ownership_table_->get_owner(table_id, page);
        const auto& owners = owner_stats.first;
        if (owners.empty()) {
            return -1; // page 还没有 owner，让调用方兜底
        }
        // exclusive 模式下 owners 只有一个；shared 模式可能多个，挑第一个即可
        return owners.front();
    }

    // 执行某个事务之后，得到了 ctid，此时需要去更新一下 smart_router 内部的 ctid 设置
    ...
    // write the backup log for prepare
    void log_prepare_result(std::string &result) {
        assert(prepare_result_logger != nullptr);
        prepare_result_logger->log_sync(result);
    }
    ...
    void run_router_worker_new() {
        assert((SYSTEM_MODE >=0 && SYSTEM_MODE <=8) || SYSTEM_MODE == 13 || SYSTEM_MODE == 101 || (SYSTEM_MODE >=23 && SYSTEM_MODE <=25)); 
            ...
            for(int t=0; t<router_worker_threads_; t++) {
                futures.emplace_back(threadpool.enqueue([this, &txn_batch, t, batch_size]() {
                    ...
                    for(size_t i = t; i < batch_size; i += router_worker_threads_) {
                        ...
                        //     }
                        // }
                        else if(SYSTEM_MODE == 2 || SYSTEM_MODE == 3 || SYSTEM_MODE == 5 || SYSTEM_MODE == 6 || SYSTEM_MODE == 7 || SYSTEM_MODE == 8 || SYSTEM_MODE == 13 || SYSTEM_MODE == 101 || (SYSTEM_MODE >= 23 && SYSTEM_MODE <= 25)) {
...
private:
    ...
    // log for prepare results
    SynchronizedLogger* prepare_result_logger; 
