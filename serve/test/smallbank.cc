...
void SmallBank::create_smallbank_stored_procedures(pqxx::connection* conn) {
    ...
    try {
            ...
            // --- Split-transaction helper procedures (for SYSTEM_MODE 101) ---
            // checking delta update
            txn.exec(R"SQL(
            CREATE OR REPLACE FUNCTION sp_subtxn_checking_delta(a1 INT, delta_val INT)
            RETURNS TABLE(rel TEXT, id INT, ctid TID, balance INT, txid BIGINT)
            LANGUAGE plpgsql AS $$
            DECLARE
                c_ctid TID;
            ...
            BEGIN
                UPDATE checking c
                SET balance = c.balance + delta_val
                WHERE c.id = a1
                RETURNING c.ctid, c.balance INTO c_ctid, c_bal;
            ...
            // savings delta update
            txn.exec(R"SQL(
            CREATE OR REPLACE FUNCTION sp_subtxn_savings_delta(a1 INT, delta_val INT)
            RETURNS TABLE(rel TEXT, id INT, ctid TID, balance INT, txid BIGINT)
            LANGUAGE plpgsql AS $$
            DECLARE
                s_ctid TID;
            ...
            BEGIN
                UPDATE savings s
                SET balance = s.balance + delta_val
                WHERE s.id = a1
                RETURNING s.ctid, s.balance INTO s_ctid, s_bal;
            ...
            // checking read helper
            txn.exec(R"SQL(
            CREATE OR REPLACE FUNCTION sp_subtxn_checking_read(a1 INT)
            RETURNS TABLE(rel TEXT, id INT, ctid TID, balance INT, txid BIGINT)
            LANGUAGE plpgsql AS $$
            DECLARE
                c_ctid TID;
            ...
            // savings read helper
            txn.exec(R"SQL(
            CREATE OR REPLACE FUNCTION sp_subtxn_savings_read(a1 INT)
            RETURNS TABLE(rel TEXT, id INT, ctid TID, balance INT, txid BIGINT)
            LANGUAGE plpgsql AS $$
            DECLARE
                s_ctid TID;
